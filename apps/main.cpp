...
auto main() -> int {
  ...
  const Camera camera = {Vec3(0.0, 0.0, -1.0), scene};
  camera.render();
  ...
  const uchar4 *image_ptr = scene->getImage();
  const std::vector<uchar4> image(image_ptr,
                                  image_ptr + image_width * image_height);
